...
public class TreeSetRecursiveUnbalanced<T> implements Iterable<T>{
	...
	public TreeSetRecursiveUnbalanced() {
		try {
	...
	public TreeSetRecursiveUnbalanced<T> balance(){
		
		ArrayList<T> alt = new ArrayList<>();
		for(T element : this) alt.add(element);
		
		TreeSetRecursiveUnbalanced<T> balanced = new TreeSetRecursiveUnbalanced<>(comparator);
		
		putArrayToTree(balanced, alt, 0, alt.size()-1);
		return balanced;
	}

	private void putArrayToTree(TreeSetRecursiveUnbalanced<T> balanced, ArrayList<T> alt, int start, int end) {
